...
- **Mutual kill is same-cell only**: the only tie is simultaneous same-cell entry — both heads moving into the same empty cell. That avoids stacking two meshes in one tile. Every other trail hit resolves normally: the rider that hit the trail loses the round, even if the opponent's head happens to be right next to the impact point.
