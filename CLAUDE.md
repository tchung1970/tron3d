...
- **Collision check** per tick: walls (out of bounds) and trail cells (own or opponent). Rounds end only when a head lands on a wall or trail — mid-air same-cell collisions are not an end condition. The arena boundary is rendered as inward-facing translucent cyan wall planes + a bright white ground rail so the player can see where they'll crash.
