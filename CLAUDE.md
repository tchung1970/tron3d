...
- **Collision check** per tick: walls (out of bounds) and trail cells (own or opponent). Rounds end only when a head lands on a wall or trail — mid-air same-cell collisions are not an end condition. The arena boundary is rendered as a tall translucent red glass wall with a yellow/black diagonal hazard band at the base and red warning rails — the tall red wall + hazard stripe make the boundary unmistakable.
- **Head-on ram is mutual**: a crash on the opponent's trail ties if either (a) the opponent's head is within Chebyshev distance 4, or (b) the crash cell shares a row/column with the opponent and they're within 8 cells on that axis. Rule (a) covers direct rams and close dodges; rule (b) covers wider evasive turns on the same line — both produce a TIE ROUND so the AI can never "dodge a head-on at the last moment" and still win.
