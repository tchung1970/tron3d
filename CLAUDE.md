...
- **Tie is same-cell only**: `TIE ROUND` only fires when both cycles' crash cells are identical — either both heads stepped into the same empty cell, or both died on the same trail cell. Any other simultaneous crash (e.g. both hit each other's trails at different cells on the same tick) resolves as AI-takes-it rather than a tie, so the player's visible mistake is never masked as a draw. The same-cell entry branch still forces both to die at the shared cell so two meshes don't render stacked in one tile.
