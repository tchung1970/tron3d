...
- **Collision check** per tick: walls (out of bounds) and trail cells (own or opponent). Rounds end only when a head lands on a wall or trail — mid-air same-cell collisions are not an end condition. The arena boundary is rendered as a translucent cyan glass wall with a yellow/black diagonal hazard band at the base and red warning rails top-bottom — the hazard stripe is the unmistakable "do not cross" marker and the glass stays low-opacity so it doesn't occlude the horizon.
- **Head-on ram is mutual**: stepping into the opponent's *current* head cell kills both cycles at that cell, even if the opponent turned away this tick. Prevents the AI from dodging a head-on by re-planning during `aiChoose()` at the top of the tick.
