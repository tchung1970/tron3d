...
- **TIE is strict head-on only**: `TIE ROUND` fires only when both heads step into the same cell on the same tick AND they're moving in directly opposite directions (i.e. literally ramming each other on the same axis), with neither having already crashed. Perpendicular same-cell convergence is not a head-on; it still kills both meshes at the shared cell to avoid stacked rendering, but the round goes to AI. Any other simultaneous crash also goes to AI — the player's visible mistake always scores as a loss, never a tie.
