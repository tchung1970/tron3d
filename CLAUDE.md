...
- **Head-on ram is mutual**: a crash on the opponent's trail ties only if the opponent's current head is within Chebyshev distance `RAM_RANGE = 2` of the crash cell. This covers direct rams and last-tick perpendicular dodges where the opponent is still adjacent. Wider perpendicular T-intersection hits are genuine side impacts and resolve normally (the rider that hit the trail loses). Simultaneous same-cell entry is separately handled as a mutual kill so two meshes never stack in one tile.
