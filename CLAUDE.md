...
- **No ties**: if the player crashed (for any reason, including a simultaneous same-cell head-on), AI takes the round. Only a pure AI-only crash gives the round to the player. The same-cell head-on branch still kills both meshes at the shared cell so they don't stack in a single tile, but scoring-wise the player is treated as having lost because they saw the mistake they made.
